...
export function BackupSettings() {
  ...
  const refresh = async () => {
    try {
      setList((await listServerBackups()).backups);
    } catch {
      setList([]);
    }
  };
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      {/* Export/Import move a backup file; Backup/Restore use the proxy's
          backup store — mirroring the WebDAV controls. */}
      <div className="flex flex-wrap gap-2">
        <FlatButton onClick={() => void exportNow()}>Export</FlatButton>
        <FlatButton onClick={() => fileInput.current?.click()}>Import</FlatButton>
        ...
        >
          <PopoverTrigger asChild>
            <FlatButton disabled={busy}>Restore</FlatButton>
          </PopoverTrigger>
          <PopoverContent align="end" className="w-80 p-0">
            <div className="label-mono border-b border-border px-3 py-2 text-muted-foreground">
              Restore a backup
            </div>
